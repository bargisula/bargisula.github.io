...
> ⚠️ **本文件是人工維護的快照，可能落後於實際系統，僅供導覽。**
> **系統的唯一真相是 `.claude/` 下的檔案**：
> - Agent 沒有中央註冊檔——每個 `.claude/agents/*.md` 靠自己的 `name:` 欄位自我註冊。看清單：`grep "^name:" .claude/agents/*.md`
> - 流程 SOP 看 `.claude/commands/*.md`（例：晨會 = `meeting.md`）
> - 要即時系統地圖（含缺口與重複），跑 `/ceo system`
>
> 自本文件最後更新後，系統已新增多層未在此反映：DSMM 傳導鏈合規閘門、insights / signals 評分、jade/inbox 璞玉收件匣、VCP 驗證點。以實際檔案為準。
...
| **CIO** | 投資長 | 整合研究部各 Agent 輸出，組成完整報告並推 GitHub；璞玉報告、個股研究的最終發布者 |
...
| 個股研究 | 每週 | `/stock-pick [TICKER]` | 論點優先，基本面+催化劑+失效條件三位一體 |
