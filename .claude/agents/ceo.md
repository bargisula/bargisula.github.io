...
> **晨會流程的唯一定義在 `.claude/commands/meeting.md`；本檔不再重複，避免兩份 SOP 打架並節省 token。**
> CEO 主持晨會時，先讀 `.claude/commands/meeting.md` 取得完整流程（後台並行收情報 → 五節晨報 → 議題自動識別 → 裁定 → 下次檢查點 → 會議紀錄雙寫），依該檔執行。
> 晨報輸出後進入自由問答時，套用下方「自由問答調度規則」。
