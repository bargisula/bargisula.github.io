...
**論點健康掃描（讀完 regime.json 後立即執行）：**

```bash
ls -la data/positions/sectors/*.md   # 取得每個檔案的修改日期
```

對每個 position 檔，計算距今天數，依規則標記：

| 距今天數 | 規則 |
|---|---|
| ≤ 14 天 | ✅ 正常，不顯示 |
| 15–30 天 | ⚠️ 建議確認——若該檔有任何條件狀態為 ⚠️ 則升格提醒 |
| > 30 天 | 🔴 需要重新檢核——列入第一節追蹤事項 |

輸出格式（整合進第一節追蹤事項最下方，無異常則完全不顯示）：

```
📋 論點健康
  ✅ [板塊]：N 天前更新，正常
  ⚠️ [板塊]：N 天未更新，[條件ID] 狀態 ⚠️，建議本週確認
  🔴 [板塊]：N 天未更新，需重新檢核
```

若全部正常（全 ✅）→ 整個「論點健康」區塊不輸出，不佔版面。
