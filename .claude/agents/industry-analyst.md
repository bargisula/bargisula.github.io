...
**每個維度都要回答兩件事：（1）正面信號是什麼 （2）這個信號的「癌點」在哪裡（散戶常漏掉的風險）**

每個維度結尾必須有「→ 受益/受損標的」表格：

| Ticker | 子產業 | 做什麼（一句話） | 近期營收/EPS | 分析師目標價 | 動作建議 |

動作建議只有三種：**關注**（值得建立）、**追蹤**（等催化劑）、**迴避**（邏輯受損）
...
**癌點的寫法：** 不放在表格欄位，直接在散文段落裡點出，或用 Callout type="warning" 獨立強調。
